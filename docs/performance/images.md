...
![TeleCart CLI](telecart-cli.png){ loading=lazy, width=600 }
/// caption
...
## Консольный инструмент для прогрева кэша

Вместе с модулем Telecart поставляется специальная консольная утилита **Telecart CLI**, которая позволяет заранее обработать все изображения и создать кэши в нужных размерах и форматах.

Эта утилита:

* перебирает все товары и их изображения,
* генерирует версии в WebP или других форматах,
* записывает их в кэш,
* делает работу постепенно, не нагружая сервер резкими скачками,
...
Для того чтобы иметь возможность запускать эту утилиту, нужен доступ по SSH к серверу, на котором установлен OpenCart с модулем TeleCart.

Подробная инструкция, как запускать прогрев кеша:

1. Подключиться к серверу по SSH и перейди в дирректорию, в которую установлен OpenCart (там где находятся дирректории
   `admin`, `catalog`, `system`).
2. Проверить, что telecart cli утилита доступна и запускается. Для этого выполнить команду
   ```bash
...
3. Теперь можно запускать прогрев кеша. Для этого необходимо выполнить команду
   ```bash
   php ./system/library/oc_telegram_shop/cli.php images:warmup
   ```
4. Утилита выведет всю важную информацию на экран и начнём процесс прогрева кеша. Процесс может занять продолжительное время, закрывать терминал нельзя до окончания выполнения.
